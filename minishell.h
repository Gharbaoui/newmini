...
/*   Updated: 2021/06/17 13:09:17 by aez-zaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/// workonline.c
int				workon_line(char *line, t_completecmd **complete,
					int numofcmds, int help);
...
int				fill_wcmd(t_workingcmds **wcmd, t_words *commands,
					int numofcmds);
int				fill_pips(t_words **pip, char *line);
int				help_short_fill_wcmd(t_workingcmds **wcmd, int numofcmds,
					t_words **pip);
int				help_fill_pipes(t_words **pip, char *line, int len);
int				fill_completecmd(t_completecmd **compcmd, t_words **pips,
					int pipindex);
...
int				h1_fill_cmdstruct(t_words **words, t_cmd **command,
					char *firstword);
int				h2_fill_cmdstruct(t_words **words, t_cmd **command,
					char *firstword, int ret);
...
int				help_make_node_env(t_fullvar **vars, t_envs **curenv,
					char *line, int *ern);
void			help2_make_node_env(t_fullvar **vars, t_envs **cuenv,
					char *line);
int				free_error(int *ern, t_envs *cuenv, int is_free);
int				help_norm_make_node(t_envs **cuenv, t_envs **or, int *eq_pos,
					char *line);
...
t_pipcommand	*expand_current_command(t_completecmd *complet,
					t_fullvar *envs);
...
int				backs_filter_str(char **str, t_envs **exenvs,
					t_words **newwords);
int				work_on_words(t_words **mod_words, t_words *words,
					t_envs **exenvs, int order);
int				filter_string(t_words **words, t_words *w, t_envs **exenvs,
					int order);
int				get_var_name(char *line, char **key);
int				is_special(char c);
int				loop_in_filter_stringh1(int *index, char *line,
					t_words **keys, t_envs **exenvs);
...
int				nonequt(char c);
void			collect_strs_h1(t_collstrs *vars, t_words **keys,
					t_envs **exenv, int order);
...
int				exec_multi_pipe(t_pipcommand *pcmd, int **pipe,
...
int				decide_in_out(int **pipes, char **files, char **ops,
					t_iter nums);
...
int				exc_one_cmd(t_onecmd cmd, int **pipe, t_iter nums,
					t_fullvar **env_var);
...
int				help1_sub_export(t_fullvar **vars, char *line, char **key,
					char **tmp);
int				help2_sub_export(t_fullvar **vars, char *key, char **tmp1,
					t_envs **cuvar1);
void			help3_sub_export(char *line, char *key, char **tmp1,
					t_envs **cuvar);
...
void			red_in_decide_files(char **fs, int **pipe, int append,
					t_iter nums);
...
int				help1_get_full_expanded(t_cmd *cmd, t_envs **exenvs,
					char **cline, int *exp);
...
int				fill_part_of_str(char *tmp, int *index, char *str,
					t_envs **exenvs);
